...
- UI: `/studio` is the single workspace (tabs deep-linked via `?tab=…`; `/transit` redirects); panel titles render once; no nested `CollapsibleSection`s; drag-rearrange only in "Arrange panels" mode.
