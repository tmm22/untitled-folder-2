...
- **Billing Service**: Stripe subscriptions + customer portal; webhook handler updates account status, trial periods, and payment failures.
...
- `invoices` (id, userId, stripeInvoiceId, amountMinorUnits, periodStart, periodEnd, status, createdAt)
- `audit_logs` (id, actorId, action, targetType, targetId, metadataJson, createdAt)

## Core Workflows
1. **Premium Activation**
   - User completes checkout → Stripe webhook marks account `active`.
...
1. Select auth + billing stack; set up Stripe sandbox and auth provider tenant.
...
6. Ship premium dashboard UI and Stripe billing flows.
