...
- **Import summaries are entitlement-gated.** `/api/imports` only runs OpenAI summarisation for BYOK keys (used directly) or managed credentials (server key); everyone else gets content only and the client calls `summarizeOnDevice` (`@/lib/summarize/onDevice`) — browser built-in AI when already available (never trigger a model download), else the deterministic extractive summarizer. `ImportedEntry.summaryEngine` records which engine produced the summary ('openai' | 'browser-ai' | 'extractive').
- **Text bound for the narration editor must be TTS-clean.** Run it through `cleanForNarration` (`@/lib/summarize/narrationClean`) — strips URLs, markdown, citations, promo/credit boilerplate, and unspeakable symbols. `summarizeOnDevice` cleans its own input and output; the Imports panel cleans on "Load into editor" and "Narrate summary". Extend `BOILERPLATE_LINE_PATTERNS` there rather than ad-hoc filtering elsewhere.
