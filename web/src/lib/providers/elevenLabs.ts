...
class ElevenLabsAdapter implements ProviderAdapter {
  ...
  async synthesize(payload: ProviderSynthesisPayload): Promise<ProviderSynthesisResponse> {
    ...
    const response = await this.requestSpeech(payload, false);
  ...
  async synthesizeStream(payload: ProviderSynthesisPayload): Promise<ProviderSynthesisStream | null> {
    ...
    const response = await this.requestSpeech(payload, true);
    if (!response.body) {
      return null;
    }
  ...
  private async requestSpeech(payload: ProviderSynthesisPayload, streaming: boolean): Promise<Response> {
    ...
    const endpoint = streaming
      ? `https://api.elevenlabs.io/v1/text-to-speech/${payload.voiceId}/stream`
