...
describe('workspace layout store', () => {
  ...
  it('moves a panel downward within the same column without jumping to the end', () => {
    const { actions } = useWorkspaceLayoutStore.getState();
    act(() => {
      actions.movePanel('capture', 'left', 3);
    });
    const layout = useWorkspaceLayoutStore.getState().layout;
    expect(layout.left).toEqual(['cleanupInstructions', 'importEntries', 'snippets', 'capture', 'transcriptHistory']);
  });
